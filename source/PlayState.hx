...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if ((unlock0 != null || unlock1 != null) && (controls.PAUSE || FlxG.keys.justPressed.SPACE)) endSong();
	...
	private var unlock0:FlxSprite;
	private var unlock1:FlxSprite;
	public function finishSong(?ignoreNoteOffset:Bool = false):Void
	{
		...
		vocals.volume = 0;
		vocals.pause();
		if (Paths.formatToSongPath(SONG.song) == "haywire" && !ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
			canPause = false;
			FlxG.sound.playMusic(Paths.music('breakfast'), 0.6);
			if (!Achievements.isAchievementUnlocked('GOODENDING') && songShocks < 40) 
			{
				unlock0 = new FlxSprite(0, 0).loadGraphic(Paths.image('unlock0'));
				unlock0.cameras = [camHUD];
				unlock0.scrollFactor.set();
				unlock0.screenCenter();
				add(unlock0);
				unlock0.loadGraphic(Paths.image('unlock0'));
			}
			else if (!Achievements.isAchievementUnlocked('BADENDING') && songShocks >= 40)
			{
				unlock1 = new FlxSprite(0, 0).loadGraphic(Paths.image('unlock1'));
				unlock1.cameras = [camHUD];
				unlock1.scrollFactor.set();
				unlock1.screenCenter();
				add(unlock1);
				unlock1.loadGraphic(Paths.image('unlock1'));
				//StoryMenuState.weekCompleted.set(WeekData.weeksList[3], true);
			} else {
	...
	public function endSong():Void
	{
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
						if (storyWeek != 1) StoryMenuState.weekCompleted.set(WeekData.weeksList[storyWeek], true);
		...
		/*if (!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false) && (!isStoryMode && Paths.formatToSongPath(SONG.song) == "haywire" || storyWeek == 1 && storyPlaylist.length <= 0)) {
			if (songShocks >= 40) StoryMenuState.weekCompleted.set(WeekData.weeksList[3], true); else if (songShocks < 40) StoryMenuState.weekCompleted.set(WeekData.weeksList[1], true);
			trace(StoryMenuState.weekCompleted.get(WeekData.weeksList[3]));
